...
class JobPostingService:
    ...
    async def create_job_posting(self, session: AsyncSession, job_posting: CreatePostingRequest) -> JobDescription:
        try:
            self.__logger.info(f"Creating job posting: {job_posting}")
            posting = JobDescription(position=job_posting.position, description=job_posting.description)
            session.add(posting)
