...
class JobPostingController(BaseController):
    ...
    async def create_job_posting(self, request: CreatePostingRequest, session: AsyncSession = Depends(get_db_session)) -> JSONResponse:
        try:
            self.__logger.info(f"Creating job posting: {request}")
            ...
            return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"message": "Error creating job posting"})

    async def list_job_postings(self, session: AsyncSession = Depends(get_db_session)) -> JSONResponse:
        try:
            self.__logger.info("Listing job postings...")
            postings = await self.__job_posting_service.list_job_postings()
            res = {"data": postings}
            ...
            return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"message": "Error listing job postings"})